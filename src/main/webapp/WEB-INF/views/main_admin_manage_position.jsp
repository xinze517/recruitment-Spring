...
<div class="d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 bg-white border-bottom shadow-sm">
    ...
    <a class="btn btn-outline-primary" href="mainPage.adminDo?tab=position-manage">返回主页</a>
</div>
<div class="container my-5">
    <form action="editPosition.adminDo" method="post">
        <label class="sr-only">
            <input name="tab" value="position-manage" hidden>
            <input name="positionId" value="${requestScope.position.position_id}" hidden>
        </label>
        <%-- 岗位状态通知 --%>
        <c:choose>
            <c:when test="${requestScope.position.status.equals('屏蔽')}">
            ...
            <c:when test="${requestScope.position.status.equals('发布')}">
